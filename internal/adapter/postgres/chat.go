...
func (r *ChatRepository) Find(ctx context.Context, chatID common.ChatID) (*chat.Settings, error) {
	row := executor(ctx, r.pool).QueryRow(ctx,
		`SELECT id, title, locale, timezone, default_topic_id, active, default_top_tier_only, stream_announcements, stream_language, quiet_from_minute, quiet_to_minute FROM telegram_chat WHERE id = $1`, chatID.Value)
	var s chat.Settings
	var id int64
	if err := row.Scan(&id, &s.Title, &s.Locale, &s.Timezone, &s.DefaultTopicID, &s.Active, &s.DefaultTopTierOnly, &s.StreamAnnouncements, &s.StreamLanguage, &s.QuietFromMinute, &s.QuietToMinute); err != nil {
...
func (r *ChatRepository) ListActive(ctx context.Context) ([]chat.Settings, error) {
	rows, err := executor(ctx, r.pool).Query(ctx,
		`SELECT id, title, locale, timezone, default_topic_id, active, default_top_tier_only, stream_announcements, stream_language, quiet_from_minute, quiet_to_minute
		   FROM telegram_chat
		  WHERE active = true
		  ORDER BY id`)
	if err != nil {
	...
	for rows.Next() {
		var s chat.Settings
		if err := rows.Scan(&s.ChatID.Value, &s.Title, &s.Locale, &s.Timezone, &s.DefaultTopicID, &s.Active, &s.DefaultTopTierOnly, &s.StreamAnnouncements, &s.StreamLanguage, &s.QuietFromMinute, &s.QuietToMinute); err != nil {
			return nil, err
		}
		out = append(out, s)
...
func (r *ChatRepository) Save(ctx context.Context, s chat.Settings) (chat.Settings, error) {
	_, err := executor(ctx, r.pool).Exec(ctx,
		`INSERT INTO telegram_chat(id, title, locale, timezone, default_topic_id, active, default_top_tier_only, stream_announcements, stream_language, quiet_from_minute, quiet_to_minute, created_at, updated_at)
		...
		s.StreamAnnouncements, s.StreamLanguage, s.QuietFromMinute, s.QuietToMinute)
	return s, err
}
...
func (r *ChatRepository) ManagedChats(ctx context.Context, userID common.UserID) ([]chat.Settings, error) {
	rows, err := executor(ctx, r.pool).Query(ctx, `
		SELECT c.id, c.title, c.locale, c.timezone, c.default_topic_id, c.active, c.default_top_tier_only, c.stream_announcements, c.stream_language, c.quiet_from_minute, c.quiet_to_minute
